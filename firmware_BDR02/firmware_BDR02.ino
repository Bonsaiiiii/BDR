...
//GERAL
String Modo, Perfil, Mac, wifiEr, Conf, voltConf, RA, userRA, passRA;
char cModo[7]; char cPerfil[20]; char cMac[6]; char cwifiEr[2]; char cConf[2]; char cvoltConf[4]; char cRA[2]; char cuserRA[15]; char cpassRA[15];
        ...
        File configFile = SPIFFS.open(filename, "r");
          if (configFile){
            ...
            if (!error){
              ...
              RA = strcpy(cRA, avancado["RA"]);
              userRA = strcpy(cuserRA, avancado["userRA"]);
              passRA = strcpy(cpassRA, avancado["passRA"]);
              return true;
          }else{
...
void saveConfigFile(String filename){
  if(filename=="/BDR.json"){
    ...
    avancado["RA"] = RA.c_str();
    avancado["userRA"] = userRA.c_str();
    avancado["passRA"] = passRA.c_str();
...
void wifisetup() {
  //Serial.println("onConf1 " + Conf);
    ...
    server.on("/configRA", HTTP_POST, [](AsyncWebServerRequest *request){
    Serial.println("Received POST request");
    if (request->hasParam("RA", true)) {
        AsyncWebParameter* ci = request->getParam("RA", true); 
        AsyncWebParameter* cj = request->getParam("userRA", true); 
        AsyncWebParameter* ck = request->getParam("passRA", true);
...
void remoteControlSetup() {
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){
  if (!request->authenticate(cuserRA, cpassRA)) {
  ...
  server.on("/remotesourcebdr", HTTP_POST, [](AsyncWebServerRequest *request){
  Serial.println("Received POST request");
  if (request->hasParam("hostNtpS", true)) {
    AsyncWebParameter* ci = request->getParam("hostNtpS", true); 
    AsyncWebParameter* cj = request->getParam("portNtpS", true); 
    AsyncWebParameter* ck = request->getParam("mtpntS", true);
    AsyncWebParameter* cl = request->getParam("sendRev2", true); 
  ...
  server.on("/getTime", HTTP_GET, [](AsyncWebServerRequest *request){
    String json = "{";
    json += "\"timeCountVal\":\"" + String(timeCountVal) + "\",";
    json += "\"timer\":\"" + String(timer) + "\",";
    json += "\"packageDisp\":" + String(packageDisp);
    json += "}";
  ...
  server.begin();
  Serial.println("Server Setup Initiated");
}
...
String CriaGGA(String lati, String logi, String alti, String tutc){
  ...
  p = p + tutc + String(".00,") + p1 + String(",0") + p2 + String(",1,12,1.2,")+ newalt + String(",M,0,M,,");
  int checksum = 0;
...
int cliente(const char* Network, const char* Password, char* Port, char* NTPHost, char* NTPUser, char* NTPPassword, const char* MACAdr, char* Latitude, char* Longitude, char* Altitude, const char* Tempoutc, String SendLocation, char* MTPT){
  ...
  switch(ntripclient.reqRaw(NTPHost,iPORT,"",NTPUser,NTPPassword,"","",MACAdr)){
    case 1: Serial.print("Não foi possivel conectar ao ");Serial.print(NTPHost);Serial.print(":");Serial.println(iPORT);
            ...
            errorClient = 1;
    break;
    case 2: Serial.println("Não houve resposta do servidor ou resposta nao compreendida");
            Serial.println("SourceTable request error");
            errorClient = 2;
    break;
  ...
  if(SendLocation=="S"){
    if(Latitude!=""&&Longitude!=""&&Altitude!=""&&Tempoutc!=""){
      ...
      switch(ntripclient.reqRaw(NTPHost,iPORT,MTPT,NTPUser,NTPPassword,nmeaGGA,SendLocation,MACAdr)){
        case 1: Serial.print("Não foi possivel conectar ao ");Serial.print(NTPHost);Serial.print(":");Serial.println(iPORT);
                ntripclient.stop();
                errorClient = 1;
        break;
    ...
    /*if (getLocalTime(&timeinfo)) {
      String tempoutc = (timeinfo.tm_hour < 10 ? "0" : "") + String(timeinfo.tm_hour) + (timeinfo.tm_min  < 10 ? "0" : "") + String(timeinfo.tm_min) + (timeinfo.tm_sec  < 10 ? "0" : "") + String(timeinfo.tm_sec);
      Tempoutc = tempoutc.c_str();
  ...
  if (errorClient == 0) {
    ...
    String enviaGGA;
    if (getLocalTime(&timeinfo)) {
      String timeUTC = (timeinfo.tm_hour < 10 ? "0" : "") + String(timeinfo.tm_hour) + (timeinfo.tm_min  < 10 ? "0" : "") + String(timeinfo.tm_min) + (timeinfo.tm_sec  < 10 ? "0" : "") + String(timeinfo.tm_sec);
      GGA = CriaGGA(Latitude,Longitude,Altitude,timeUTC);
      enviaGGA = GGA + "\r\n";
